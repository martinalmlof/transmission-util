...
def main():
    ...
    for torrent in old_ssd_torrents:
        ...
        print(f"Moving torrent #{torrent.id} '{torrent.name}' from '{torrent.download_dir}' to '{new_path}'", flush=True)
        c.move_torrent_data(torrent.id, new_path, timeout = (5, 600))
